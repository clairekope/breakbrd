...
for i, sub_id in enumerate(sub_ids):
    ...
    # half_light[0,i] = cam0[ np.argwhere(cam0_id==sub_id)].flatten() \
    #                        if (cam0_id==sub_id).any() else np.nan
    # half_light[1,i] = cam1[np.argwhere(cam1_id==sub_id)].flatten() \
    #                        if (cam1_id==sub_id).any() else np.nan
    # half_light[2,i] = cam2[np.argwhere(cam2_id==sub_id)].flatten() \
    #                        if (cam2_id==sub_id).any() else np.nan
    # half_light[3,i] = cam3[np.argwhere(cam3_id==sub_id)].flatten() \
    #                        if (cam3_id==sub_id).any() else np.nan
