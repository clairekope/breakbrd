...
#################
# Because I want variants on the SFR plot
#################
sfr_denom = "total_SFR" # "total_SFR" "disk_SFR" "outer_SFR" 
...
full_denom_inst_sfr = np.empty_like(full_inner_inst_sfr)

full_inner_inst_ssfr = np.empty_like(full_inner_inst_sfr)
...
denom_inst_sfr = np.empty_like(inner_inst_sfr)
...
cntrl_denom_inst_sfr = np.empty_like(cntrl_inner_inst_sfr)

cntrl_inner_inst_ssfr = np.empty_like(cntrl_inner_inst_sfr)
cntrl_total_inst_ssfr = np.empty_like(cntrl_inner_inst_sfr)
...
for i, k in enumerate(all_inner_gas_info.keys()):
    # Full means parent (which includes D4000)
    full_inner_inst_sfr[i] = all_inner_gas_info[k]['inner_SFR'].value
    full_total_inst_sfr[i] = all_inner_gas_info[k]['total_SFR'].value
    full_denom_inst_sfr[i] = all_inner_gas_info[k][sfr_denom].value
    
    full_inner_inst_ssfr[i] = all_inner_gas_info[k]['inner_sSFR'].value
    full_total_inst_ssfr[i] = all_inner_gas_info[k]['total_sSFR'].value
...
# Calculate ratios
full_sfr_ratio = full_inner_inst_sfr/full_denom_inst_sfr
sfr_ratio = inner_inst_sfr/denom_inst_sfr
cntrl_sfr_ratio = cntrl_inner_inst_sfr/cntrl_denom_inst_sfr

if sfr_denom == "disk_SFR" or sfr_denom == "outer_SFR":
    full_sfr_ratio = np.log10(full_sfr_ratio)
    sfr_ratio = np.log10(sfr_ratio)
    cntrl_sfr_ratio = np.log10(cntrl_sfr_ratio)
...
full_ssfr_ratio = full_inner_inst_ssfr/full_total_inst_ssfr
ssfr_ratio = inner_inst_ssfr/total_inst_ssfr
cntrl_ssfr_ratio = cntrl_inner_inst_ssfr/cntrl_total_inst_ssfr
...
if sfr_denom == "total_SFR":
    good = np.nonzero(full_total_inst_sfr) # Some SFR values are garbage
    n_lvls = 50 # For contours
    ylim = (0,1)
else:
    good = np.isfinite(full_sfr_ratio)
    n_lvls = 20
    ylim = (-4, 2 if sfr_denom=="disk_SFR" else 4)
...
ax[1,3].hist(sfr_ratio[np.isfinite(sfr_ratio)], 
             density=True, orientation='horizontal', histtype='step', lw=2)
ax[1,3].hist(cntrl_sfr_ratio[np.isfinite(cntrl_sfr_ratio)], 
             density=True, orientation='horizontal', 
             histtype='step', lw=2, ls='--')
ax[1,3].set_ylim(*ylim)
ax[1,3].yaxis.set_visible(False)

ax[1,0].set_ylabel("{}Instantaneous SFR(fiber) / SFR({})".format(
                     "log " if sfr_denom!="total_SFR" else "", sfr_denom[:-4]))
fig.savefig("sfr_ratio_z00_{}.png".format(sfr_denom[:-4]))

########################
# Inst sSFR Concentration
########################

# Some sSFR values are garbage
good = np.logical_and(np.isfinite(full_ssfr_ratio), full_ssfr_ratio > 0)
...
# Stellar mass vs sSFR ratio
counts, binsx, binsy = np.histogram2d(log_full_mstar[good], np.log10(full_ssfr_ratio[good]), 30)
ax[0,0].hist(log_full_mstar[good], density=True, color='lightgray')
ax[0,0].hist(log_mstar, density=True, histtype='step', lw=2)
ax[0,0].hist(log_cntrl_mstar, density=True, histtype='step', lw=2, ls="--")
ax[0,0].xaxis.set_visible(False)
...
ax[1,0].contourf(counts.T, n_lvls-1, colors=gy_colors,
                   extent=[binsx.min(),binsx.max(),binsy.min(),binsy.max()])
b = ax[1,0].scatter(log_mstar, np.log10(ssfr_ratio), marker='.')
c = ax[1,0].scatter(log_cntrl_mstar, np.log10(cntrl_ssfr_ratio), marker='.')
ax[1,0].set_xlabel("$\mathrm{\log_{10}(M_*)\ [M_\odot]}$")
ax[1,0].set_xlim(10,12)
#ax[1,0].set_ylim(0,1)

# Inst SFR vs sSFR ratio
counts, binsx, binsy = np.histogram2d(np.log10(full_total_inst_sfr[good]),
                                      np.log10(full_ssfr_ratio[good]), 30)
ax[0,1].hist(np.log10(full_total_inst_sfr[good]), density=True, color='lightgray')
ax[0,1].hist(np.log10(total_inst_sfr), density=True, histtype='step', lw=2)
ax[0,1].hist(np.log10(cntrl_total_inst_sfr), density=True, histtype='step', lw=2, ls="--")
ax[0,1].xaxis.set_visible(False)
...
ax[1,1].contourf(counts.T, n_lvls-1, colors=gy_colors,
                   extent=[binsx.min(),binsx.max(),binsy.min(),binsy.max()])
ax[1,1].scatter(np.log10(total_inst_sfr), np.log10(ssfr_ratio), marker='.')
ax[1,1].scatter(np.log10(cntrl_total_inst_sfr), np.log10(cntrl_ssfr_ratio), marker='.')
ax[1,1].set_xlabel("$\log_{10}$ Instantaneous SFR(total)")
ax[1,1].set_xlim(-1.5,1.5)
#ax[1,1].set_ylim(0,1)
ax[1,1].yaxis.set_visible(False)

# Inst sSFR vs sSFR ratio
counts, binsx, binsy = np.histogram2d(np.log10(full_total_inst_ssfr[good]),
                                      np.log10(full_ssfr_ratio[good]), 30)
ax[0,2].hist(np.log10(full_total_inst_ssfr[good]), density=True, color='lightgray')
ax[0,2].hist(np.log10(total_inst_ssfr), density=True, histtype='step', lw=2)
...
ax[1,2].contourf(counts.T, n_lvls-1, colors=gy_colors,
                   extent=[binsx.min(),binsx.max(),binsy.min(),binsy.max()])
ax[1,2].scatter(np.log10(total_inst_ssfr), np.log10(ssfr_ratio), marker='.')
ax[1,2].scatter(np.log10(cntrl_total_inst_ssfr), np.log10(cntrl_ssfr_ratio), marker='.')
ax[1,2].set_xlabel("$\log_{10}$ Instantaneous sSFR(total)")
ax[1,2].set_xlim(-12.5,-9)
#ax[1,2].set_ylim(0,1)
ax[1,2].yaxis.set_visible(False)
...
ax[1,3].hist(np.log10(full_ssfr_ratio[good]),
             density=True, orientation='horizontal', color='lightgray')
ax[1,3].hist(np.log10(ssfr_ratio),
             density=True, orientation='horizontal', histtype='step', lw=2)
ax[1,3].hist(np.log10(cntrl_ssfr_ratio), density=True, orientation='horizontal', 
             histtype='step', lw=2, ls='--')
#ax[1,3].set_ylim(0,1)
ax[1,3].yaxis.set_visible(False)

ax[1,0].set_ylabel("log Instantaneous sSFR(fiber) / sSFR(total)")
fig.savefig("ssfr_ratio_z00.png")
